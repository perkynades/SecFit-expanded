...
async function deleteExercise(id) {
	...
	if (!response.ok) {
		...
		const alert = createAlert(`Could not delete exercise ${id}`, data); //eslint-disable-line no-undef
...
async function retrieveExercise(id) {
	...
	if (!response.ok) {
		const data = await response.json();
		const alert = createAlert('Could not retrieve exercise data!', data); //eslint-disable-line no-undef
		document.body.prepend(alert);
		return;
	}
	document.querySelector('select').removeAttribute('disabled');
...
async function updateExercise(id) {
	...
	const body = {'name': formData.get('name'), 
		...
		'unit': formData.get('unit')};
	const response = await sendRequest('PUT', `${HOST}/api/exercises/${id}/`, body); //eslint-disable-line no-undef

	if (!response.ok) {
		const data = await response.json();
		const alert = createAlert(`Could not update exercise ${id}`, data); //eslint-disable-line no-undef
		document.body.prepend(alert);
		return;
	}
