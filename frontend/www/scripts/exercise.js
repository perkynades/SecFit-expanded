...
async function createExercise() {
	...
	let response = await sendRequest('POST', `${HOST}/api/exercises/`, body); //eslint-disable-line no-undef

	if (response.ok) {
		window.location.replace('exercises.html');
	} else {
		let data = await response.json();
		let alert = createAlert('Could not create new exercise!', data); //eslint-disable-line no-undef
...
async function deleteExercise(id) {
	let response = await sendRequest('DELETE', `${HOST}/api/exercises/${id}/`); //eslint-disable-line no-undef
	if (!response.ok) {
		let data = await response.json();
		let alert = createAlert(`Could not delete exercise ${id}`, data); //eslint-disable-line no-undef
		document.body.prepend(alert);
	} else {
		window.location.replace('exercises.html');
	}
...
async function retrieveExercise(id) {
	let response = await sendRequest('GET', `${HOST}/api/exercises/${id}/`); //eslint-disable-line no-undef

	console.log(response.ok);

	if (!response.ok) {
		let data = await response.json();
		let alert = createAlert('Could not retrieve exercise data!', data); //eslint-disable-line no-undef
		document.body.prepend(alert);
	} else {
		document.querySelector('select').removeAttribute('disabled');
...
async function updateExercise(id) {
	...
	let body = {'name': formData.get('name'), 
		...
		'unit': formData.get('unit')};
	let response = await sendRequest('PUT', `${HOST}/api/exercises/${id}/`, body); //eslint-disable-line no-undef

	if (!response.ok) {
		let data = await response.json();
		let alert = createAlert(`Could not update exercise ${id}`, data); //eslint-disable-line no-undef
		document.body.prepend(alert);
	} else {
